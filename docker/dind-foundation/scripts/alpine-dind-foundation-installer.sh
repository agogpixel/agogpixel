...
ALPINE_DIND_FOUNDATION_INSTALLER_ENTRYPOINT_PATH="${ALPINE_FOUNDATION_ENTRYPOINTD_PATH}"/dind-entrypoint.sh
...
alpine_dind_foundation_installer_main() {
  ...
  alpine_dind_foundation_installer_setup_entrypoint
...
alpine_dind_foundation_installer_parse_args() {
  ...
  local longopts="${ALPINE_DIND_FOUNDATION_INSTALLER_OPT_HELP[LONG]},${ALPINE_DIND_FOUNDATION_INSTALLER_OPT_USERNAME[LONG]}:,${ALPINE_DIND_FOUNDATION_INSTALLER_OPT_INSTALL_DOCKER_COMPOSE[LONG]}"
...
alpine_dind_foundation_installer_setup_entrypoint() {
  ...
  echo "$(alpine_dind_foundation_installer_get_entrypoint)" > "${ALPINE_DIND_FOUNDATION_INSTALLER_ENTRYPOINT_PATH}"
  chmod 755 "${ALPINE_DIND_FOUNDATION_INSTALLER_ENTRYPOINT_PATH}"
}
...
#!/bin/sh
exec 2>&1
exec /usr/bin/dockerd --log-driver syslog
EOF
...
alpine_dind_foundation_installer_get_entrypoint() {
  cat <<EOF
#!/bin/bash
...
if [ "\${ALPINE_FOUNDATION_RUNNING_AS_DAEMON}" = true ]; then
  # Init will handle docker daemon startup.
...
sudo_if /usr/bin/dockerd > /dev/null 2>&1 &
