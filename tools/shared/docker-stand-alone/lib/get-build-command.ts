...
import { BuildCommand, dockerString, VariantBuildConfig } from './models';
 ...
 */
export function getBuildCommand(imageName: string, projectPath: string, config: VariantBuildConfig): BuildCommand {
  const cliContext = normalize(`${projectPath}/${config.context}`);
  const cliDockerfile = ['-f', normalize(`${projectPath}/${config.dockerfile}`)];
  ...
  const cliTags = [
    ...(
      variantName !== 'default' ? ['-t', variantTag, '-t', `${variantTag}-latest`] : []
    ),
    '-t',
    `${imageName}:latest`
  ];
